# Physics-guided correction under model misspecification (PI-DeepONet) — baseline
...
This is a self-contained implementation: a serial PI-DeepONet where the physics
loss's misspecified operator `N₀` is applied to the sum of the prior and
correction networks (`N₀[G_θ + G_ψ]`) rather than to the prior alone. See the top
level [README](../README.md) for how this folder relates to other work in the
repo. Everything needed to run it lives under this directory — code, datasets,
and cached data — so it can be run with `cd baseline` and nothing else.

The paper's method is a physics-informed DeepONet that corrects a **misspecified
governing equation** (e.g. collapsing a reaction term to a constant). This
implements it on three of the paper's four benchmarks and adds several
